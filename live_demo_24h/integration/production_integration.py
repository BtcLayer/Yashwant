...
from live_demo.security.data_sanitizer import (
    DataSanitizer,
    sanitize_log_record,
    hash_identifiers,
)
...
class ProductionIntegration:
    ...
    def log_market_data(
        ...
        # Sanitize data
        sanitized_record = sanitize_log_record(
            validation_result["validated"], "market_data"
        )
    ...
    def log_signals(
        ...
        # Sanitize data
        sanitized_record = sanitize_log_record(
            validation_result["validated"], "signals"
        )
    ...
    def log_ensemble(
        ...
        raw_record = {
            ...
            "bandit_arm": (
                bandit_state.get("chosen_arm", "model") if bandit_state else "model"
            ),
            "bandit_weights": bandit_state.get("weights", {}) if bandit_state else {},
            "reward_prev": (
                bandit_state.get("last_reward", 0.0) if bandit_state else 0.0
            ),
            "regret_est": 0.0,
            "exp_rate": 0.0,
            "a": -8.5,
        ...
        # Sanitize data
        sanitized_record = sanitize_log_record(
            validation_result["validated"], "ensemble"
        )
    ...
    def log_execution(
        ...
        # Sanitize data
        sanitized_record = sanitize_log_record(
            validation_result["validated"], "execution"
        )
