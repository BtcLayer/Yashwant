...
class ExecutionTracker:
        ...
        return [
            {
                ...
                "execution_time_ms": (
                    (time.time() - exec_record["start_time"]) * 1000
                    if exec_record["completed"]
                    else None
                ),
            }
    ...
    def get_execution_log(
        ...
        for exec_record in reversed(self.execution_history):
            if (
                exec_record["execution"].asset == asset
                and exec_record["execution"].bar_id == bar_id
            ):
    ...
    def calculate_impact(
        self, order_size: float, fill_price: float, market_price: float
    ) -> float:
    ...
    def get_performance_metrics(self) -> Dict[str, Any]:
        ...
        for exec_record in completed_executions:
            ...
            if execution.fill_px and execution.limit_px:
                slippage = self.calculate_slippage(
                    execution.limit_px, execution.fill_px
                )
        ...
        metrics = {
            "execution_success_rate": len(completed_executions)
            / max(self.total_orders, 1),
    ...
    def _calculate_route_performance(self) -> Dict[str, Any]:
        ...
        for route in self.route_stats.keys():
            ...
            if route_executions:
                avg_time = sum(
                    e["execution"].ioc_ms or 0 for e in route_executions
                ) / len(route_executions)
