...
async def fetch_addr(
    session: aiohttp.ClientSession, addr: str, start_ms: int, end_ms: int
) -> Dict[str, Any]:
    payload = {
    ...
    async with aiohttp.ClientSession() as session:
        for label, win in WINDOWS.items():
            ...
            actives_btc = sum(
                1
                for o in outs
                if isinstance(o.get("fills_btc"), int) and o["fills_btc"] > 0
            )
            total_btc = sum(int(o.get("fills_btc") or 0) for o in outs)
            actives_any = sum(
                1
                for o in outs
                if isinstance(o.get("fills_any"), int) and o["fills_any"] > 0
            )
            total_any = sum(int(o.get("fills_any") or 0) for o in outs)
            errs = sum(
                1
                for o in outs
                if str(o.get("status")).startswith("err")
                or o.get("status") not in (200,)
            )
