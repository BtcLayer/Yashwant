...
class DataSanitizer:
    ...
    def sanitize_data(self, data: Any, context: Optional[str] = None) -> Any:
        ...
        if isinstance(data, dict):
            return {key: self.sanitize_data(value, f"{context}.{key}" if context else key) 
        ...
        for rule in self.rules:
            ...
            if matches:
                ...
                if rule.hash_salt:
                    ...
                    sanitized = re.sub(rule.pattern, rule.replacement, sanitized)
                    self._log_detection(rule, matches[0] if matches else "", "", context)
    ...
    def _log_detection(self, rule: SanitizationRule, original: str, hashed: str, context: Optional[str]):
    ...
    def mask_sensitive_fields(self, data: Dict[str, Any], sensitive_fields: Optional[List[str]] = None) -> Dict[str, Any]:
        ...
        for key, value in masked_data.items():
            if isinstance(value, dict):
                ...
                masked_data[key] = [
                    self.mask_sensitive_fields(item, sensitive_fields) if isinstance(item, dict) else item
                    for item in value
                ]
    ...
    def validate_data_classification(self, data: Dict[str, Any]) -> Dict[str, SensitivityLevel]:
            ...
            if isinstance(value, str):
                ...
                for rule in self.rules:
                    if re.search(rule.pattern, value):
                        ...
                        elif rule.sensitivity.value == "confidential" and max_sensitivity != SensitivityLevel.RESTRICTED:
                            max_sensitivity = SensitivityLevel.CONFIDENTIAL
                        elif rule.sensitivity.value == "internal" and max_sensitivity == SensitivityLevel.PUBLIC:
        ...
        for value in data.values():
            if isinstance(value, str):
                ...
                if nested_sensitivity.value == "restricted":
                    return SensitivityLevel.RESTRICTED
                elif nested_sensitivity.value == "confidential" and max_sensitivity != SensitivityLevel.RESTRICTED:
                    max_sensitivity = SensitivityLevel.CONFIDENTIAL
                elif nested_sensitivity.value == "internal" and max_sensitivity == SensitivityLevel.PUBLIC:
                    max_sensitivity = SensitivityLevel.INTERNAL
    ...
    def get_stats(self) -> Dict[str, Any]:
        ...
        for detection in recent_detections:
            ...
            stats["detection_types"][data_type] = stats["detection_types"].get(data_type, 0) + 1
            stats["sensitivity_breakdown"][sensitivity] = stats["sensitivity_breakdown"].get(sensitivity, 0) + 1
...
def sanitize_log_record(record: Dict[str, Any], context: Optional[str] = None) -> Dict[str, Any]:
    ...
    for field in id_fields:
        if field in hashed_record and hashed_record[field]:
            if 'wallet' in field or 'address' in field:
                hashed_record[field] = sanitizer.hash_wallet_address(hashed_record[field])
            elif 'user' in field:
