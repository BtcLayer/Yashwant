...
from .log_emitter import get_emitter
...
class LogRouter:
    ...
    def __init__(self, logging_cfg: Dict[str, Any]):
    ...
    def emit_execution(self, *, ts: Optional[float], asset: str, exec_resp: Dict[str, Any], risk_state: Dict[str, Any], bar_id: Optional[int] = None):
        ...
        if targets.get("llm"):
            try:
                ...
                write_jsonl("execution_log", rec, asset=asset)
            except Exception:
                pass
    ...
    def emit_costs(self, *, ts: Optional[float], asset: str, costs: Dict[str, Any]):
        ...
        if targets.get("llm"):
            try:
                write_jsonl("costs_log", {**costs, "asset": asset, "event_id": event_id}, asset=asset)
            except Exception:
                pass
    ...
    def emit_equity(self, *, asset: str, ts: Optional[float], pnl_total_usd: Optional[float], equity_value: Optional[float], realized_return_bps: Optional[float] = None):
        ...
        if targets.get("llm"):
            try:
                write_jsonl("pnl_equity_log", {**payload, "event_id": event_id}, asset=asset)
            except Exception:
                pass
    ...
    def emit_overlay_status(self, *, ts: Optional[float], asset: str, status: Dict[str, Any]):
        ...
        if targets.get("llm"):
            try:
                write_jsonl("overlay_status", {**payload, "event_id": event_id}, asset=asset)
            except Exception:
                pass
    ...
    def emit_alert(self, *, ts: Optional[float], asset: str, alert: Dict[str, Any]):
        ...
        if targets.get("llm"):
            try:
                write_jsonl("alerts", {**payload, "event_id": event_id}, asset=asset)
            except Exception:
                pass
    ...
    def emit_hyperliquid_fill(self, *, ts: Optional[float], asset: str, fill: Dict[str, Any]):
        ...
        if targets.get("llm"):
            try:
                write_jsonl("hyperliquid_fills", {**payload, "event_id": event_id}, asset=asset)
            except Exception:
                pass
