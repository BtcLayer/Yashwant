...
class ProductionLogEmitter:
    ...
    def _start_background_writers(self):
        ...
        for log_type in ["market", "signals", "ensemble", "risk", "execution", "costs", "health", "repro", "order_intent", "feature_log", "calibration"]:
    ...
    def _write_single_with_retry(self, log_type: str, record: Dict[str, Any]):
        ...
        for attempt in range(self.config.retry_attempts):
            try:
                ...
                self.logger.warning(f"Write attempt {attempt + 1} failed for {log_type}: {e}")
    ...
    def _rotate_file(self, log_type: str, file_path: str):
        ...
        try:
            ...
            rotated_path = base_path.parent / f"{base_path.stem}_{timestamp}{base_path.suffix}"
        ...
        for log_type in self._queues:
            ...
            if os.path.exists(file_path):
                stats['files'][log_type] = {
                    ...
                    'last_modified': datetime.fromtimestamp(os.path.getmtime(file_path), IST).isoformat()
                }
...
def get_production_emitter(config: Optional[EmitterConfig] = None) -> ProductionLogEmitter:
