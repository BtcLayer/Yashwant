...
class ReproTracker:
    ...
    def get_training_dates(self) -> tuple[Optional[str], Optional[str]]:
        ...
        try:
            if os.path.exists(self.model_path):
                with open(self.model_path, 'r') as f:
                    manifest = json.load(f)
                    train_start = manifest.get('train_start', '2025-07-01T00:00:00+05:30')
    ...
    def validate_reproducibility(self) -> Dict[str, Any]:
        ...
        validation = {
            ...
            'training_dates_present': config.train_start_ist is not None and config.train_end_ist is not None,
