...
async def run_live_with_overlay(config_path: str, dry_run: bool = False):
    ...
    one_shot = bool(
        cfg.get("execution", {}).get("one_shot", False)
        or os.environ.get("LIVE_DEMO_ONE_SHOT")
    )
    force_validation = bool(
        cfg.get("execution", {}).get("force_validation_trade", False)
    )
...
>>>>>>> a425beb9a39dcb2c03ba879f40b73a3beb6babde
    ...
    try:
        ...
        try:
            ...
            raise ImportError(
                "Neither binance-connector nor python-binance is available"
            ) from e
            ...
            async with aiohttp.ClientSession() as session:
                async with session.get(url, params=params) as resp:
                    if resp.status == 200:
                        ...
                        net = sum(
                            float(t["qty"]) if t["m"] else -float(t["qty"])
                            for t in data
                        )
                        return net
        except Exception:
                ...
                if exec_result:
                    ...
                    if bandit and last_chosen_arm:
                        ...
                        if last_close_value is not None:
                            ...
                            )  # Reward if direction was correct
                            bandit.update(last_chosen_arm, reward)

                    last_close_value = c
                    last_chosen_arm = (
                        overlay_decision.alignment_rule
                    )  # Use alignment rule as arm
        ...
        try:
            ...
            logger.buffer(
                ...
                row=[
                    ...
                    confidence,
                    overlay_decision.alignment_rule,
                    str(overlay_decision.chosen_timeframes),
                ],
            )
