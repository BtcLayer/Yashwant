...
async def run_live(config_path: str, dry_run: bool = None):
    ...
    # Force LLM log bar_id mapping to 1h bars
    try:
        os.environ['LLM_FREQ_MINUTES'] = '60'
    except Exception:
        pass
    # Ensure timeframe-specific PAPER_TRADING_ROOT when not explicitly provided
    try:
        if not os.environ.get('PAPER_TRADING_ROOT'):
            # Resolve to <repo>/paper_trading_outputs/1h (inside repo)
            repo_root = os.path.abspath(os.path.join(os.path.dirname(__file__), os.pardir))
            tf_root = os.path.join(repo_root, 'paper_trading_outputs', '1h')
    ...
    try:
        ...
        pass
    # Debug: record credentials path and existence for diagnosis
    try:
        with open(os.path.join(tf_root, 'live_errors.log'), 'a', encoding='utf-8') as fh:
            fh.write(f"[SheetsLogger][1h] creds_path={creds_path} exists={os.path.exists(creds_path) if creds_path else None}\n")
    except Exception:
        pass
    ...
    log_router = LogRouter(cfg.get('logging', {}), bot_version='1h', base_root=os.path.join(tf_root, 'logs'))
    # Initialize unified emitter once for this run (used in multiple places)
    try:
        emitter = get_emitter('1h', base_dir=os.path.join(tf_root, 'logs'))
    except Exception:
        emitter = None
    ...
    # Track last decision for reward update (bandit) and safety init
    ...
    async with local_HL(hl_ws if hl_ws else "wss://stub", addresses=addresses, coin='BTC', mode='public_trades') as hl:
        ...
        while True:
            ...
            # Write heartbeat
            try:
                write_heartbeat(paper_root(), bot_version='1h', last_bar_id=ts, last_trade_ts=last_ts)
            except Exception:
                pass
            ...
            if 'bandit' in locals() and locals().get('bandit') is not None and last_close_value is not None and locals().get('last_chosen_arm') is not None:
                try:
                    ...
                    if arm_index is not None:
                        ...
                        if locals().get('bandit_io') is not None:
            ...
            last_chosen_arm = decision.get('details', {}).get('chosen') if 'bandit' in locals() and locals().get('bandit') is not None else None
