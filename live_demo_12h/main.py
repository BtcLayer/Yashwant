...
def load_config(path: str) -> Dict:
    """Load primary config, then shallow-merge optional fallbacks/overrides without breaking existing behavior.

    Order of precedence (later wins):
      1) primary path (required)
      2) sibling config_overlay.json (if present) for overlay-specific keys
      3) live_demo/config/config.local.json (if present) for environment overrides
    """
    with open(path, 'r', encoding='utf-8') as fp:
        cfg = json.load(fp)
    # Optional: merge overlay config if present
    try:
        ...
        if os.path.exists(overlay_path):
            with open(overlay_path, 'r', encoding='utf-8') as fov:
                ov = json.load(fov)
            # Only merge known overlay-related keys to avoid surprising overrides
        ...
        # Non-fatal: ignore overlay fallback errors
        pass
    # Optional: environment-local overrides
    try:
        ...
        # Non-fatal: ignore local override errors
        pass
    return cfg
...
async def run_live(config_path: str, dry_run: bool = False):
    ...
    # Force LLM log bar_id mapping to 12h bars
    try:
        os.environ['LLM_FREQ_MINUTES'] = '720'
    except Exception:
        pass
    # Ensure timeframe-specific PAPER_TRADING_ROOT when not explicitly provided
    try:
        if not os.environ.get('PAPER_TRADING_ROOT'):
            # Resolve to <repo>/paper_trading_outputs/12h (inside repo)
            repo_root = os.path.abspath(os.path.join(os.path.dirname(__file__), os.pardir))
            tf_root = os.path.join(repo_root, 'paper_trading_outputs', '12h')
    ...
    # (removed duplicate project_root assignment from merge)
    def abspath(p: str):
        return (
            p
    ...
    try:
        ...
        pass
    try:
        with open(os.path.join(tf_root, 'live_errors.log'), 'a', encoding='utf-8') as fh:
            fh.write(f"[SheetsLogger][12h] creds_path={creds_path} exists={os.path.exists(creds_path) if creds_path else None}\n")
    except Exception:
        pass
    logger = SheetsLogger(creds_path, sheet_id, headers=headers, root_dir=tf_root)
    logger.ensure_headers()
    log_router = LogRouter(cfg.get('logging', {}), bot_version='12h', base_root=os.path.join(tf_root, 'logs'))
    try:
        emitter = get_emitter('12h', base_dir=os.path.join(tf_root, 'logs'))
    except Exception:
        emitter = None

    def paper_root() -> str:
        repo_root2 = os.path.abspath(os.path.join(os.path.dirname(__file__), os.pardir))
        repo_paper = os.path.abspath(os.path.join(repo_root2, 'paper_trading_outputs'))
    ...
    if offline:
        # Stub HyperliquidListener to a no-op async context manager
        class _HLStub:
            ...
            async def stream(self):
                if False:
                    yield {}
        # Monkey-patch name to use stub inside this function scope
        local_HL = _HLStub
        # Stub funding fetch to avoid network
        ...
        try:
            ...
            pass
        # Provide a single kline from warmup as the last closed bar
        try:
            ...
            # Fallback synthetic row if warmup missing expected columns
            import time as _t
        ...
        try:
            ...
            pass
        # Override fallback public mood helper to no-op
    ...
    async with local_HL(hl_ws, addresses=addresses, coin='BTC', mode='public_trades') as hl:
        ...
        async def _poll_user_fills_by_time(ts_end_ms: int, interval_ms: int):
            # In offline mode, skip network and return empty
            if offline:
                return []
        ...
        while True:
            ...
            for fill_row in drained_fills:
                try:
                    ...
                    pass
                # Maintain existing Sheets shape for downstream compatibility
                try:
                    logger.buffer(
                        tab=cfg['sheets']['tabs']['hyperliquid'],
                        row=[to_iso(fill_row.get('ts')), fill_row.get('ts'), fill_row.get('address'), fill_row.get('coin'), fill_row.get('side'), fill_row.get('price'), fill_row.get('size')]
                    )
                except Exception:
                    pass
            ...
            try:
                _health_preds.append((float(model_out.get('p_down', 0.0)), float(model_out.get('p_up', 0.0))))
            except Exception:
                pass
            try:
            ...
            # Bandit selection log (event: select)
            ...
            try:
                ...
                # Use current model calibration params if present; fallback to defaults
                ...
                # Use the same s_model source as used for decision (BMA-adjusted when enabled)
                s_model = float((decision_model_out if 'decision_model_out' in locals() else model_out).get('s_model', 0.0)) if isinstance(model_out, dict) else 0.0
                pred_cal_bps = 10000.0 * (a + (b * s_model))
                # Update health with in-band flag
                try:
                    health_monitor.update_inband(pred_cal_bps, band_bps)
                except Exception:
                    pass
                # Emit compact calibration log for LLM context
                try:
                ...
                if abs(pred_cal_bps) <= band_bps:
                    # Enforce neutral when prediction is in the no-trade band
            ...
            if bar_count < warm_skip:
                ...
                if decision['dir'] == 0 and force_validation and not used_force:
                    decision = {**decision, 'dir': 1, 'alpha': max(0.05, abs(model_out.get('s_model', 0.1)))}
                    used_force = True
                ...
                if stopped_for_day:
                    ...
                    tgt = risk.target_position(decision['dir'], decision['alpha']) if decision['dir'] != 0 else 0.0
                    ...
                    if mode == 'passive_then_cross':
                        ...
                        # Notify throttle counter about an order attempt
            ...
            if bandit is not None and last_close_value is not None and last_chosen_arm is not None:
                try:
                    ...
                    reward = r * (last_exec_pos if isinstance(last_exec_pos, (int, float)) else 0.0)
                    chosen = last_chosen_arm
                    # Map chosen arm name to index
                    # 4-arm index mapping (mood removed)
                    ...
                    if arm_index is not None:
                        ...
                        # Bandit update log (event: update)
                        logger.buffer(
                            tab=cfg['sheets']['tabs'].get('bandit', 'bandit'),
                            row=[
                                to_iso(ts), ts,
                                'update',
                                chosen,
                                '',  # raw_val not applicable on update
                                '',  # dir not applicable on update
                                '',  # alpha not applicable on update
                                '', '', '', '',  # elig flags (pros, amateurs, model_meta, model_bma)
                                float(reward),
                        ...
                        # (debug CSV update logging removed)
            ...
            last_chosen_arm = decision.get('details', {}).get('chosen') if bandit is not None else None
            ...
            try:
                ...
                unrealized = (c - paper_avg_px) * paper_qty if abs(paper_qty) > 1e-12 else 0.0
                ...
                try:
                    ...
                    dd_pct = 100.0 * (session_peak_equity - equity) / max(1e-9, session_peak_equity)
                ...
                if equity_tab:
                    logger.buffer(
                        tab=equity_tab,
                        row=[to_iso(ts), ts, c, paper_qty, paper_avg_px, realized, unrealized, equity]
                    )
            ...
            if exec_resp:
                _qty_val_m = exec_resp.get('qty') if isinstance(exec_resp, dict) else 0.0
                _px_val_m = exec_resp.get('price') if isinstance(exec_resp, dict) else 0.0
                _notional_m = abs(float(_qty_val_m) * float(_px_val_m)) if _qty_val_m is not None and _px_val_m is not None else ''
                _intended_notional = abs(float(tgt) * float(risk.cfg.base_notional)) if 'tgt' in locals() else ''
                logger.buffer(
                    tab=cfg['sheets']['tabs']['mirror'],
                    row=[
                        to_iso(ts), ts, decision['dir'], decision['alpha'], c,
                        _notional_m,
                        _intended_notional,
                        tgt, risk.get_position(),
                        exec_resp.get('exch_qty_before') if isinstance(exec_resp, dict) else '',
                        exec_resp.get('exch_qty_after') if isinstance(exec_resp, dict) else '',
                        exec_resp.get('reconciled') if isinstance(exec_resp, dict) else '',
                ...
                try:
                    # Compute notionals
                    _qty_val = exec_resp.get('qty') if isinstance(exec_resp, dict) else 0.0
                    _px_val = exec_resp.get('price') if isinstance(exec_resp, dict) else 0.0
                    _notional = abs(float(_qty_val) * float(_px_val)) if _qty_val is not None and _px_val is not None else ''
                    _intended = abs(float(tgt) * float(risk.cfg.base_notional)) if 'tgt' in locals() else ''
                    logger.buffer(
...
if __name__ == '__main__':
    ...
    except (RuntimeError, OSError, ValueError, KeyError, ImportError, gspread.exceptions.APIError) as exc:
        try:
