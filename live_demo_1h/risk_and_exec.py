...
class RiskConfig:
    ...
    rebalance_min_pos_delta: float = 0.0  # min change in target position fraction to trade
...
<<<<<<< HEAD
        ...
        if old_qty == 0.0 or (old_qty > 0 and signed_trade > 0) or (old_qty < 0 and signed_trade < 0):
...
<<<<<<< HEAD
    def mirror_to_exchange(self, target_pos: float, last_price: float, dry_run: bool = True) -> Optional[Dict]:
        """
        ...
        if abs(float(target_pos) - float(self._pos)) < max(0.0, float(self.cfg.rebalance_min_pos_delta or 0.0)):
=======
...
<<<<<<< HEAD
        ...
        except (ImportError, AttributeError):  # pragma: no cover - missing package or api change
=======
            mod = importlib.import_module("binance.error")
            ClientError = getattr(mod, "ClientError")  # type: ignore
...
>>>>>>> a425beb9a39dcb2c03ba879f40b73a3beb6babde
        ...
        try:
<<<<<<< HEAD
            resp = self.client.new_order(symbol=self.symbol, side=side, type='MARKET', quantity=qty)
...
<<<<<<< HEAD
        ...
        bid_qty = float(book.get('bid_qty')) if (book and book.get('bid_qty') is not None) else 0.0
        ask_qty = float(book.get('ask_qty')) if (book and book.get('ask_qty') is not None) else 0.0
        if side == 'BUY':
=======
        bid = float(book.get("bid")) if (book and book.get("bid") is not None) else None
        ask = float(book.get("ask")) if (book and book.get("ask") is not None) else None
...
<<<<<<< HEAD
        # Build response
        avg_px = (weighted_price_numer / max(1e-9, total_exec_qty)) if total_exec_qty > 0 else last_price
=======
        avg_px = (
