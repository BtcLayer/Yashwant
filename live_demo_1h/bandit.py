...
<<<<<<< HEAD
        self.counts = np.array(counts, dtype=float) if counts is not None else np.zeros(self.n_arms, dtype=float)
        self.means = np.array(means, dtype=float) if means is not None else np.zeros(self.n_arms, dtype=float)
        self.variances = np.array(variances, dtype=float) if variances is not None else np.ones(self.n_arms, dtype=float)
=======
        self.counts = (
