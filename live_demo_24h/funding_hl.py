...
class FundingHL:
    ...
    def _write_debug(self, record: Dict[str, Any]) -> None:
        ...
        try:
            # Prefer unified PAPER_TRADING_ROOT if set; otherwise resolve to <repo>/paper_trading_outputs
            env_root = os.environ.get('PAPER_TRADING_ROOT')
            if env_root:
                # If env points to a timeframe subdir (e.g., .../paper_trading_outputs/5m), write at its parent root
