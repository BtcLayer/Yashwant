...
class FeatureLogger:
    ...
    def log_features(
        ...
        mid_price = market_data.get('mid', 0.0)
        volume = market_data.get('volume', 0.0)
        spread_bps = market_data.get('spread_bps', 0.0)
        rv_1h = market_data.get('rv_1h', 0.0)
    ...
    def get_feature_log(self, ts: float, asset: str, bar_id: int) -> Optional[Dict[str, Any]]:
    ...
    def log_features_dict(
        ...
        feature_log = self.log_features(ts, bar_id, asset, market_data, features, feature_names)
        if feature_log:
            return {
