...
def compute_signals_and_eligibility(
    ...
    th: Thresholds,
<<<<<<< HEAD
) -> Tuple[Dict[str, float], Dict[str, bool], Tuple[float, float, float, float], Dict[str, float]]:
...
=======
    ...
    eligible = {
        "pros": abs(s_top) >= th.S_MIN,
        "amateurs": abs(s_bot) >= th.S_MIN,
        "mood": abs(s_mood) >= th.M_MIN,
        "model": (conf_model >= th.CONF_MIN) and (alpha_model >= th.ALPHA_MIN),
    }
    side_eps_vec = (th.S_MIN, th.S_MIN, th.M_MIN, th.ALPHA_MIN)
    extras = {"conf_model": conf_model, "alpha_model": alpha_model}
>>>>>>> a425beb9a39dcb2c03ba879f40b73a3beb6babde
...
=======
    arm_order = ["pros", "amateurs", "mood", "model"]
    sig_order = ["S_top", "S_bot", "S_mood", "S_model"]
>>>>>>> a425beb9a39dcb2c03ba879f40b73a3beb6babde
    emask = np.array([bool(eligible[a]) for a in arm_order], dtype=bool)
    # Fast exit: no arms eligible
    if not np.any(emask):
<<<<<<< HEAD
        return {'dir': 0, 'alpha': 0.0, 'details': {'chosen': None, 'eligible': eligible, 'signals': signals}}
...
=======
            if (
                float(model_optimism) != 0.0
                and hasattr(bandit, "means")
                and len(getattr(bandit, "means", [])) >= 4
            ):
                bandit.means[3] = float(bandit.means[3]) + float(model_optimism)
                bumped = True
            chosen_idx = int(bandit.select(emask))
            ...
            if bumped:
                bandit.means[3] = float(bandit.means[3]) - float(model_optimism)
    chosen_arm = (
        arm_order[chosen_idx] if 0 <= chosen_idx < len(arm_order) else arm_order[0]
    )
>>>>>>> a425beb9a39dcb2c03ba879f40b73a3beb6babde
...
<<<<<<< HEAD
        if chosen_arm.startswith('model'):
            ...
            alpha_base = max(extras.get('conf_model', 0.0), extras.get('alpha_model', 0.0))
=======
        if chosen_arm == "model":
            alpha_base = max(
...
<<<<<<< HEAD
def decide(cohort_snapshot: Dict, model_out: Dict, th: Thresholds, bandit: object, *, epsilon: float = 0.0, model_optimism: float = 0.0) -> Dict:
