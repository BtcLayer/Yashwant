...
class PnLAttributionTracker:
    ...
    def _calculate_alpha_pnl(self, position_change: float, price_change: float) -> float:
    ...
    def _calculate_timing_pnl(self, position_change: float, price_change: float) -> float:
    ...
    def get_current_attribution(self) -> PnLAttribution:
        ...
        return PnLAttribution(
            ...
            total=self.cumulative_alpha + self.cumulative_timing + self.cumulative_fees + self.cumulative_impact
        )
    
    def get_attribution_log(self, timestamp: float, asset: str, bar_id: int) -> Dict[str, Any]:
    ...
    def _calculate_attribution_quality(self) -> float:
        ...
        non_zero_attributions = sum(1 for a in self.attribution_history 
                                  if abs(a['attribution'].total) > 0.01)
    ...
    def get_attribution_breakdown(self, window: int = 100) -> Dict[str, Any]:
        ...
        return {
            ...
            'alpha_sharpe': np.mean(alpha_values) / np.std(alpha_values) if np.std(alpha_values) > 0 else 0,
            'timing_sharpe': np.mean(timing_values) / np.std(timing_values) if np.std(timing_values) > 0 else 0
        }
    ...
    def get_performance_attribution(self) -> Dict[str, Any]:
        ...
        return {
            ...
            'performance_analysis': {
                ...
                'cost_contribution': stats.get('fees_percentage', 0) + stats.get('impact_percentage', 0),
