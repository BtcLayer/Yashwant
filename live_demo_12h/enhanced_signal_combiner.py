...
class EnhancedSignalCombiner:
    ...
    def combine_signals(self, signal_result: OverlaySignalResult) -> CombinedSignal:
        ...
        for rule_config in self.alignment_rules:
            ...
            if not self._has_required_timeframes(signals, rule_config.required_timeframes):
                continue
    ...
    def _has_required_timeframes(self, signals: Dict[str, OverlaySignal], 
                                required: List[str]) -> bool:
        """Check if we have signals for all required timeframes"""
        return all(tf in signals for tf in required)
    
    def _apply_alignment_rule(self, signals: Dict[str, OverlaySignal], 
                             rule_config: AlignmentConfig) -> Optional[CombinedSignal]:
    ...
    def _apply_agreement_rule(self, signals: Dict[str, OverlaySignal], 
                            rule_config: AlignmentConfig) -> Optional[CombinedSignal]:
        ...
        if len(set(non_zero_directions)) == 1:
            ...
            confidences = [s.confidence for s in required_signals if s.direction == direction]
    ...
    def _apply_neutral_override_rule(self, signals: Dict[str, OverlaySignal], 
                                   rule_config: AlignmentConfig) -> Optional[CombinedSignal]:
        ...
        if abs(override_signal.alpha) < rule_config.threshold:
            other_signals = {tf: signals[tf] for tf in signals.keys() 
                           if tf != override_timeframe}
    ...
    def _apply_weighted_average_rule(self, signals: Dict[str, OverlaySignal], 
                                  rule_config: AlignmentConfig) -> Optional[CombinedSignal]:
    ...
    def _apply_majority_vote_rule(self, signals: Dict[str, OverlaySignal], 
                                 rule_config: AlignmentConfig) -> Optional[CombinedSignal]:
        ...
        majority_signals = [s for s in signals.values() if s.direction == majority_direction]
    ...
    def _apply_threshold_rule(self, signals: Dict[str, OverlaySignal], 
                             rule_config: AlignmentConfig) -> Optional[CombinedSignal]:
    ...
    def _create_fallback_signal(self, signals: Dict[str, OverlaySignal]) -> CombinedSignal:
