...
class HealthMonitor:
    ...
    def update_predictions(self, prediction: float, actual_return: float, timestamp: float):
        ...
        if len(self.predictions_history) >= 2:
            ...
            actuals = [a['actual_return'] for a in list(self.actual_returns_history)[-20:]]
            if len(preds) == len(actuals) and len(preds) > 1:
                ic = np.corrcoef(preds, actuals)[0, 1] if not np.isnan(np.corrcoef(preds, actuals)[0, 1]) else 0.0
    ...
    def check_leakage(self, prediction: float, actual_return: float, timestamp: float) -> bool:
    ...
    def calculate_hit_rate(self, window: int) -> Optional[float]:
        ...
        if len(self.predictions_history) < window or len(self.actual_returns_history) < window:
            return None
        
        recent_preds = [p['prediction'] for p in list(self.predictions_history)[-window:]]
        recent_actuals = [a['actual_return'] for a in list(self.actual_returns_history)[-window:]]
    ...
    def calculate_turnover(self) -> Optional[float]:
        ...
        recent_turnover = [t['turnover_bps'] for t in list(self.turnover_history)[-288:]]  # Last day
    ...
    def calculate_capacity_participation(self, adv_usd: float) -> Optional[float]:
        ...
        recent_positions = [p['position'] for p in list(self.positions_history)[-288:]]  # Last day
    ...
    def get_health_metrics(self) -> HealthMetrics:
        ...
        return HealthMetrics(
            ...
            capacity_participation=self.calculate_capacity_participation(25000000),  # Default ADV
