...
from live_demo.reason_codes import VetoReasonCode, GuardReasonCode

def save_health_snapshot(log_dir="paper_trading_outputs/5m/logs"):
    ...
    # Updated to find all signals.jsonl files recursively (handling date partitions)
    signals_files = []
    order_intent_files = []
                ...
                for line in f:
                    ...
                    try:
                        ...
                        if data.get("side") == "HOLD":
                            ...
                            # Threshold is common, check it first or just count all failures?
                            # Let's count all FALSE conditions as contributing vetoes
                            vetoed_by = []
            ...
            with open(s_path, "r", encoding="utf-8") as f:
                for line in f:
                    ...
                    try:
                        ...
                        # (Pre-trade vetoes handled by order_intent above)
                        if decision_dir == 0 and mode:
                            # Try to match against GuardReasonCode
                            try:
                                reason = mode
                                stats["vetos"][reason] = stats["vetos"].get(reason, 0) + 1
                            except Exception:
    ...
    print(f"📊 Summary: {stats['streams']['signals']} bars, {stats['streams']['orders']} orders. Vetoes: {veto_summary}. Staleness: {stats['staleness_ms']}ms")
