"""
PRODUCTION RETRAINING SCRIPT - 5M MODEL V3
==========================================
FIXED VERSION - All issues resolved

Changes from V2:
- Label encoding fixed (sklearn compatible)
- Unicode characters removed
- Proper probability indexing
...
print("="*80)
print("PRODUCTION 5M MODEL RETRAINING V3")
print("="*80)
...
def fetch_data():
    """Load data from CSV (API fallback built-in)"""
    
    # Use existing CSV data
    csv_files = ['ohlc_btc_5m.csv', 'ohlc_btc_5m_complete.csv']
    ...
    raise FileNotFoundError("No suitable CSV data found")
...
def adaptive_labeling(df, forward_bars=12):
    ...
    # Label (using -1, 0, 1 initially)
    df['target_raw'] = 0  # Neutral
    df.loc[df['forward_return'] > df['threshold'], 'target_raw'] = 1  # UP
    df.loc[df['forward_return'] < -df['threshold'], 'target_raw'] = -1  # DOWN
    
    # Remap to sklearn format: -1->0 (DOWN), 0->1 (NEUTRAL), 1->2 (UP)
...
def compute_class_weights(y, target_dist={0: 0.40, 1: 0.20, 2: 0.40}):
...
print(f"[OK] Computed sample weights for 40/20/40 target distribution")
...
print("[OK] Meta-classifier trained")
...
def validate_model(X_meta, y_true, name="Test"):
    ...
    # Probabilities: [0]=DOWN, [1]=NEUTRAL, [2]=UP
    p_down = proba[:, 0]
    p_neutral = proba[:, 1]
    p_up = proba[:, 2]
    ...
    direction_sklearn = np.argmax(proba, axis=1)  # 0, 1, 2
    direction = direction_sklearn - 1  # Convert back to -1, 0, 1 for reporting
...
print("\n[OK] ALL REQUIREMENTS MET - MODEL READY FOR DEPLOYMENT")
...
if Path(MODEL_DIR, 'LATEST.json').exists():
    ...
    print(f"[OK] Backed up current model to {backup_path}")
...
print(f"[OK] Saved {model_files['meta_classifier']}")

# Save calibrator
joblib.dump(calibrator, Path(MODEL_DIR, model_files['calibrator']))
print(f"[OK] Saved {model_files['calibrator']}")

# Save feature columns
with open(Path(MODEL_DIR, model_files['feature_columns']), 'w') as f:
    json.dump(FEATURE_COLS, f)
print(f"[OK] Saved {model_files['feature_columns']}")
